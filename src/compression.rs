...
mod tests {
    ...
    fn encode_medium_number() {
        assert_eq!(128u8.encode(), vec![0x80, 1]);
    }

    #[test]
    fn encode_large_number() {
        assert_eq!(16383u16.encode(), vec![0xFF, 0x7F]);
    }

    #[test]
    fn encode_very_large_number() {
        assert_eq!(2097151u32.encode(), vec![0xFF, 0xFF, 0x7F]);
    }

    #[test]
    fn encode_huge_number() {
        assert_eq!(268435455u64.encode(), vec![0xFF, 0xFF, 0xFF, 0x7F]);
    }

    #[test]
    fn encode_extremely_large_number() {
        assert_eq!(34359738367u64.encode(), vec![0xFF, 0xFF, 0xFF, 0xFF, 0x7F]);
    }
}
